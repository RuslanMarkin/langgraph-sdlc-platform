...
draft PR. Первый коммит содержит только `docs/sdlc/<feature>.md` с
утверждёнными артефактами; продуктовый код не меняется, а merge недоступен
агенту. Для этого GitHub Actions получает только права **Contents: write** и
**Pull requests: write**.
