...
После финального утверждения планов event-driven runner создаёт от
`SDLC_BASE_BRANCH` (по умолчанию `main`) изолированную ветку `sdlc/...` и
draft PR. Первый коммит содержит `docs/sdlc/<feature>.md` с утверждёнными
артефактами. Затем отдельный агент реализации получает спецификацию, план
разработки и QA-план из LangGraph и может добавить ровно один unified diff.

Этот агент не исполняет команды из ответа модели: он применяет diff только в
ветке `sdlc/...`, только к пересечению файлов из утверждённого плана и заранее
заданного списка `--evidence`. Нельзя изменять `.github`, секреты, Git-данные,
файлы зависимостей и lock-файлы, создавать, удалять, переименовывать файлы или
менять их режим. После применения запускаются фиксированные команды
`pnpm check` и `pnpm test`; только успешный результат может быть закоммичен и
отправлен в draft PR. Результат, версия prompt и вызов модели видны в Langfuse.

Агент не переводит PR из draft, не делает review и не выполняет merge. В
GitHub для `main` должна быть включена защита: pull request, успешный `check`,
решённые обсуждения и минимум один approving review человека. После проверки
изменений владелец вручную переводит PR в **Ready for review**, оставляет
GitHub review **Approve** и выполняет merge обычными средствами GitHub.
